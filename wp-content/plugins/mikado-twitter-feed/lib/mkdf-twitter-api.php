...
class MikadofTwitterApi {
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = __('Internal WP error', 'mkd');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = __('Redirect URL couldn\t not be generated', 'mkd');
                    } else {
                    ...
                    $responseObj->status = false;
                    $responseObj->message = __('Couldn\'t connect with Twitter API', 'mkd');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = __('Access token obtained', 'mkd');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = __('Authorize token and it\'s secret were not obtainer', 'mkd');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = __('Couldn\'t connect with Twitter', 'mkd');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = __('It seams like you haven\t connected with your Twitter account', 'mkd');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = __('Couldn\'t retreive content from database', 'mkd');
            }
        }
