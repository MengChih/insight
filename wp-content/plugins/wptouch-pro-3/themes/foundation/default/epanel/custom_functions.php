...
if ( ! function_exists( 'author_info' ) ){
	function author_info() {
		if ( $post == '' ) global $post;
			
		$content= $post->post_content;
		$pos = mb_strpos($content, '文\\');
		if ($pos === false) $pos = mb_strpos($content, '文＼');
		if ($pos === false) $pos = mb_strpos($content, '文／');
		if ($pos === false) $pos = mb_strpos($content, '文/');
		if ($pos === false) return '';

		$info = mb_substr($content, $pos ,30);
		$pos = mb_strpos($info, '<');
		$info = mb_substr($info, 0 , $pos);
...
if ( ! function_exists( 'truncate_utf8' ) ){
   function truncate_utf8($len,$str){
	if(function_exists("mb_strlen")){
            $sum=0;$chrsum=0;$addstr=0;
                $sum2=0;$chrsum2=0;
                for($i=0;$i<mb_strlen($str,'UTF-8');$i++){
                    $t=mb_substr($str,$i,1,'UTF-8');
                    if($sum<=($len-1.5)){
                        if(strlen($t)==1){$sum=$sum+0.5;}else{$sum=$sum+1;}
                        $chrsum=$chrsum+1;
                        $addstr=1;
                    }else{
                        if(strlen($t)==1){$sum2=$sum2+0.5;}else{$sum2=$sum2+1;}
                        $chrsum2=$chrsum2+1;
                        if($sum2>1.5){break;}
                    }
                }
                if(($sum+$sum2)<=$len){$addstr=0;}
                //echo $sum."|".$sum2;
                if($addstr==1){$str = mb_substr($str,0,$chrsum,'UTF-8')."...";}
                else{}
		
		return $str;
        }else{
            if(strlen($str)>=$len){return substr($str,0,$len-1)."...";
            }else{return $str;}        
        }
   }
}
...
if ( ! function_exists( 'truncate_post' ) ){
	function truncate_post($amount,$echo=true,$post='') {
		...
		if (et_get_option($shortname.'_use_excerpt') == 'on' && $postExcerpt <> '') { 
			$truncate = $postExcerpt;	
		} 
	
			if ( strlen($truncate) <= $amount*3 ) $echo_out = ''; else $echo_out = '...';
			...
			$truncate = strip_tags($truncate,'<br>'); 
	/*
			//if ($echo_out == '...') $truncate = substr($truncate, 0, strrpos(substr($truncate, 0, $amount), ' '));
			if ($echo_out == '...')  $truncate = substr($truncate, 0, ($amount-1)*3);
			else $truncate = substr($truncate, 0, $amount*3);
	*/
			$truncate = truncate_utf8($amount, $truncate);
...
if ( ! function_exists( 'truncate_title_new' ) ){
	function truncate_title_new($len,$echo=true,$post='',$mode="ch"){
//code by arrack
		if ( $post == '' ) $str = main_title(false); 
		else $str = $post->post_title; 
        //if($striptag==1){$str=strip_tags($str);}
		
        $str = truncate_utf8($len,$str);
...
if ( ! function_exists( 'brow_title' ) ){
	function brow_title($echo=true) {
		$truncate = get_the_title();
		
		$head = strpos($truncate,'[');
		$tail = strpos($truncate,']');
		if($head === false) {
			$head = strpos($truncate,'【');
			$tail = strpos($truncate,'】');
			if ($head !== false) $head += 3;
		}else{
			$head+=1;
		}
		$brow = '　';
		if ($head !== false)
			$brow = substr($truncate, $head, $tail - $head);
		
		if($echo) echo $brow;
		else return $brow;
	}
}
if ( ! function_exists( 'main_title' ) ){
	function main_title($echo=true) {
		$truncate = get_the_title();
		
		$tail = strpos($truncate,']');
		if($tail === false) {
			$tail = strpos($truncate,'】');
			if ($tail !== false) $tail += 3;
		}else{
			$tail+=1;
		}
		if ($head !== false)
			$truncate = substr($truncate, $tail);
...
if ( ! function_exists( 'print_thumbnail' ) ){
	function print_thumbnail($thumbnail = '', $use_timthumb = true, $alttext = '', $width = 100, $height = 100, $class = '', $echoout = true, $forstyle = false, $resize = true, $post='') {
		...
		$fileName = end(preg_split("/[-_]+/", $new_method_thumb));
		$temp = preg_split("/[x.]+/", $fileName);

//		echo $fileName.$temp[0].'kerker'.$temp[1];
		$width1 = intVal($temp[0]);
		$height1 = intVal($temp[1]);
		if($width1 > 0 && $height1 > 0){
			$w = ((float)$width)/$width1;
			$h = ((float)$height)/$height1;
			$t = ($w<$h) ? $w : $h;
			if($t>0){
				$width = $width1 * $t;
				$height = $height1 * $t;
...
if ( ! function_exists( 'print_thumbnail_to_show' ) ){
	function print_thumbnail_to_show($thumbnail = '', $use_timthumb = true, $alttext = '', $width = 100, $height = 100, $class = '', $echoout = true, $forstyle = false, $resize = true, $post='') {
		global $shortname;
		if ( $post == '' ) global $post;
