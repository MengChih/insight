...
if (!function_exists('nsGetYTThumb')){function nsGetYTThumb($yt) {  
  ...
  if (is_wp_error($response) || $response['response']['code']!='200' ) { $out = 'http://img.youtube.com/vi/'.$yt.'/sddefault.jpg';  
    $response  = nxs_remote_get($out); if (is_wp_error($response) || $response['response']['code']!='200' ) $out = 'http://img.youtube.com/vi/'.$yt.'/0.jpg';
  } return $out;  
}}
...
if (!function_exists('nxs_chckRmImage')){function nxs_chckRmImage($url, $chType='head'){ if( ini_get('allow_url_fopen')=='1' && @getimagesize($url)!==false) return true;
  $hdrsArr = nxs_getNXSHeaders(); $nxsWPRemWhat = 'nxs_remote_'.$chType; $rsp  = $nxsWPRemWhat($url, array('headers' => $hdrsArr));  
...
if (!function_exists("nxs_mkShortURL")) { function nxs_mkShortURL($url, $postID=''){ $rurl = '';  global $plgn_NS_SNAutoPoster;  if (!isset($plgn_NS_SNAutoPoster)) return; $options = $plgn_NS_SNAutoPoster->nxs_options;
    ...
    if ($options['nxsURLShrtnr']=='C' && trim($options['clkimAPIKey']!='')) {    
      $response  = nxs_remote_get('http://clk.im/api?api='.$options['clkimAPIKey'].'&url='.urlencode($url), nxs_mkRemOptsArr(''));
...
//## V4 Future
 ...
 function nxs_showNTFilters($nt, $ii, $options){ if (empty($options['fltrs'])) $options['fltrs'] = ''; $isFOn = !empty($options['fltrsOn']) && (int)$options['fltrsOn'] == 1; ?> <div style="float: right;"> <a href="http://www.nextscripts.com/snap-features/filters" target="_blank">Instructions</a> </div><h3 style="padding-left: 0px;font-size: 16px;"> 
