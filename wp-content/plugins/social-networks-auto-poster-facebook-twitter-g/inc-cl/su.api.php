...
if (!class_exists('nxsAPI_SU')){class nxsAPI_SU{ var $ck = array(); var $ckey=''; var $clid='';  var $debug = false; var $proxy = array();
    ...
    function check($u=''){ $ck = $this->ck; if ($this->debug) echo "[SU] Checking <br/>\r\n"; var_dump($ck); if (!empty($ck) && is_array($ck)) { $hdrsArr = $this->headers('https://www.stumbleupon.com/su'); 
        ...
        $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, '', $this->proxy);  $response = nxs_remote_get('https://www.stumbleupon.com/api/v2_0/user/ ', $advSet); if (is_wp_error($response)) return 'Connection ERROR: '.print_r($response, true);
    ...
    function connect($u,$p){ $badOut = 'Error: '; 
      ...
      $response = nxs_remote_get('http://www.stumbleupon.com', array('headers' => $hdrsArr)); $p = substr($p, 0, 16); if (is_wp_error($response)) return 'Connection ERROR: '.print_r($response, true);
    ...
    function post($msg, $lnk, $cat, $tags, $nsfw=false){ $ck = $this->ck; if ($this->debug) echo "[SU] Posting ...".$lnk."<br/>\r\n"; $badOut = '';  $msg = str_replace("\n",'\n', str_replace("\r",'', strip_tags($msg)));
      ...
      $response = nxs_remote_get('http://www.stumbleupon.com/submit?_nospa=true&_notoolbar=true&_notoolbar=true&_nospa=true', array( 'method' => 'GET', 'timeout' => 45, 'redirection' => 0,  'headers' => $hdrsArr, 'cookies' => $ck));   
      ...
      if ( isset($resp['_reason']) && is_array($resp['_reason']) && count($resp['_reason'])>0 && stripos($resp['_reason'][0]['message'], 'Failed to add URL')!==false) { sleep(5);
        $r2 = nxs_remote_post('https://www.stumbleupon.com/api/v2_0/submit', $advSet); if (is_wp_error($r2)) return "Connection ERROR. ".print_r($r2, true);$resp = json_decode($r2['body'], true);
      }  
...
if (!class_exists("nxs_class_SNAP_SU")) { class nxs_class_SNAP_SU {
    ...
    function doPostToNT($options, $message){ global $nxs_suCkArray; $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
      ...
      $nt = new nxsAPI_SU(); if (!empty($ck)) $nt->ck = $ck;  $loginErr = $nt->connect($options['uName'], $pass);      
